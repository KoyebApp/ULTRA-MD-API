...
async function tiktokDl(url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!res) {
        console.error('No valid data received:', response.data); // Log the raw response for debugging
        return reject(new Error('Invalid response data or no data found'));
      }

      // Check if the essential properties are present in the response
      ...
      console.error('Error in fetching TikTok video:', e.message); // Log the error for debugging
