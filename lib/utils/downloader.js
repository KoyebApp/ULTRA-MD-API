...
async function fbvdl(url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!response.ok) {
        return reject(new Error('Failed to fetch Facebook video'));
      }
      ...
      if (!title) {
        return reject(new Error('Title not found on the page'));
      }
      ...
      console.error('Error in Facebook video download:', error.message); // Log the error
      reject(new Error('Error fetching Facebook video: ' + error.message));
    }
  });
}
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!res.size && !res.images) {
        console.error('Response data is incomplete or invalid:', res);
        return reject(new Error('Missing video or image data in response'));
      }
...
async function getJooxMusic(query) {
  try {
    ...
    throw new Error('Joox search failed: ' + error.message); // Throw a descriptive error
  }
}
