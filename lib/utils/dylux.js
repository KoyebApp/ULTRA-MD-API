...
// Define the async function for handling Google Drive file download
async function Gdrive(req, res, query) {
  // Extract the URL from the query parameter
  const url = req.query.url;  // Get the URL parameter from the request query
  ...
  if (!url) {
    return res.json({
      status: false,
      creator: "Qasim Ali",
      message: "Please provide a valid Google Drive file URL."
    });
  }

  try {
    // Fetch the file download data using the provided URL
    let data = await fg.GDriveDl(url);

    // Send the download data or download link in the response
    res.json({
      status: true,
      creator: "Qasim Ali",
      data: data  // Send the data containing the download link or file info
    });
    ...
    // Send a response with the error message and stack trace for debugging
    res.json({
      status: false,
      message: "Failed to download the file.",
      error: error.message,  // Send the error message
      stack: error.stack    // Optionally, send the stack trace for debugging purposes
    });
  }
}
