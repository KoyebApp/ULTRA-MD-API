...
async function Gdrive(req, res, query) {
  ...
  if (!url) {
    return res.json({
      status: false,
      creator: "Qasim Ali",
      message: "No URL provided. Please provide a valid Google Drive file URL."
    });
  }

  try {
    // Fetch the file download data using the provided URL
    let downloadResponse = await fg.GDriveDl(url);
    ...
    res.json({
      status: true,
      creator: "Qasim Ali",
      data: downloadData  // Send the data containing the download link or file info
    });
