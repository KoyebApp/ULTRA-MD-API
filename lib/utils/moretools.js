...
// Function to fetch Pinterest media (images and videos) based on a search query
function pinterest(query) {
  return new Promise((resolve, reject) => {
    axios.get(`https://id.pinterest.com/search/pins/?autologin=true&q=${query}`, {
      ...
      // Extract image URLs
      $('div > a').get().map(b => {
        const imgLink = $(b).find('img').attr('src');
        if (imgLink) imageResults.push(imgLink);
      });

      // Debug: Log image URLs to see what we are extracting
      console.log("Extracted Image URLs:", imageResults);

      // Extract video URLs (assuming <video> tags or similar attributes)
      $('video').get().map(v => {
        const videoLink = $(v).find('source').attr('src');
        if (videoLink) videoResults.push(videoLink);
      });

      // Debug: Log video URLs to see if we're extracting them
      console.log("Extracted Video URLs:", videoResults);

      // Modify and clean image URLs
      const cleanedImages = imageResults.map(v => v.replace(/236/g, '736'));

      // Combine images and videos into a final result
      const allResults = {
